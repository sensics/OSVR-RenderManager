...
// Set up a handler to cause us to exit cleanly.
    ...
    auto startFrames = std::chrono::high_resolution_clock::now();
    ...
    while (!quit) {
        ...
        // Print timing info
        auto nowFrames = highresclock::now();
        std::chrono::duration<double> duration = nowFrames - startFrames;
        countFrames++;
        if (duration.count() >= 2.0) {
            std::cout << "Rendering at " << countFrames / duration.count()
                      << " fps" << std::endl;
    ...
    // Closing the Renderer interface cleanly is handled by the unique_ptr hrer
    // - you may have to do it yourself in your own app.
